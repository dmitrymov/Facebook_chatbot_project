...
namespace faceTry
{
	public class FacebookChatClient
	{
		...
		{
			xcc.Send(new Message(new Jid(_contacts.First(x => x.Value.name == receiverName).Key),
		...
		private void UpdateUserList(object sender, Presence pres)
		{
			...
			if (pres.Type == PresenceType.available && !Contacts.ContainsKey(pres.From.Bare))
			{
				...
				xcc.MessageGrabber.Add(new Jid(pres.From.Bare), new BareJidComparer(),
											new MessageCB(MessageReceived),null);
				if (OnUserAdded != null)
					OnUserAdded(User);
			}
